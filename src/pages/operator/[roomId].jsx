  ...
  useEffect(() => {
    ...
    async function connect() {
      try {
        ...
        room.on(RoomEvent.Reconnected, () => { setStatus('Conectado'); upd(); });
        room.on(RoomEvent.Disconnected, () => { setStatus('Reconectando...'); reconnectRef.current = setTimeout(async () => { try { await connect(); } catch (e) { setStatus('Error: ' + e.message); } }, 3000); });
    ...
    <div style={{ padding: '1rem' }}>
      ...
      {streamers.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
          {streamers.map(p => (
            <div key={p.identity} onClick={() => sel(p.identity)} style={{ border: selected === p.identity ? '2px solid #6c5ce7' : '2px solid #333', borderRadius: '8px', padding: '1rem', cursor: 'pointer', background: selected === p.identity ? '#1a1a2e' : '#111' }}>
