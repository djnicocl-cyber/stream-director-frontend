import{useState,useEffect,useRef}from'react';import{useRouter}from'next/router';import{Room,RoomEvent}from'livekit-client';
const B=process.env.NEXT_PUBLIC_BACKEND_URL||'https://stream-director-backend-production.up.railway.app';
...
const r=await fetch(B+'/api/token/operator',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({roomId})});
...
room.on(RoomEvent.DataReceived,(data)=>{try{const msg=JSON.parse(new TextDecoder().decode(data));if(msg.type==='selected')setSelected(msg.identity);}catch(e){}});
await room.connect(LK,token);setStatus('Conectado');upd();
}catch(e){setStatus('Error: '+e.message);}}
connect();return()=>{roomRef.current?.disconnect();};},[roomId]);
async function sel(identity){
await fetch(B+'/api/rooms/'+roomId+'/select',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({participantIdentity:identity})});
...
<div><h1 style={{fontSize:'1.5rem'}}>Panel del Operador</h1>
<p style={{color:'#aaa'}}>Sala: {roomId} - {status}</p></div>
