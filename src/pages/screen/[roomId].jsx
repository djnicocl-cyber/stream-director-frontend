...
export default function ScreenPage() {
  ...
  useEffect(() => {
        ...
        let pollInterval;
                                      ...
                                      try {
                                                    ...
                                                    if (r.ok) {
                                                                    ...
                                                                    if (d.selected) {
                                                                                      ...
                                                                                      checkAndAttach(room, d.selected);
                                                                    }
                                                    }
                                      } catch (e) {}
                          }, 3000);
          ...
          if (forcedSelected) {
                    target = participants.find(p => p.identity === forcedSelected);
          } else {
                    setSelected(prev => {
                                const sel = prev;
                                target = participants.find(p => p.identity === sel);
                                return prev;
                    });
          }

          if (!target && participants.length > 0) target = participants[0];
  ...
  return (
        <div style={{ width: '100vw', height: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
          ...
          {selected && (
                  <div style={{ position: 'absolute', bottom: '16px', left: '16px', background: 'rgba(0,0,0,0.6)', color: 'white', padding: '6px 16px', borderRadius: '20px', fontSize: '13px' }}>
                    {selected}
                  </div>div>
              )}
        </div>div>
